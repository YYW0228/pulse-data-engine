...
"""
import gzip, logging, os, shutil, time
...
class BackupManager:
    ...
    def backup_local(self) -> Path:
        ...
        logger.info(f"本地备份: {backup_file.name} ({db_size/1024:.0f}KB → {compressed/1024:.0f}KB, "
                    f"{ratio:.0f}%, {elapsed:.1f}s)")
        return backup_file

    def backup_remote(self, backup_file: Optional[Path] = None) -> Optional[str]:
        """备份到 Cloudflare R2"""
        ...
        try:
            ...
            if not token or not account_id:
                logger.warning(f"CLOUDFLARE_API_TOKEN 或 CF_ACCOUNT_ID 未设置, 跳过 R2 备份")
                return None
    ...
    def restore(self, source: str, target_db: Optional[str] = None) -> Path:
        """从备份文件恢复
