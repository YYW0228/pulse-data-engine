"""测试: 监控告警 + 质量 SLA + 断点续传"""
from pulse.monitor import Monitor, QualitySLA, QualityStatus, AlertLevel
...
class TestQualitySLA:
    def test_empty_db(self):
        ...
        sla.con.execute("CREATE TABLE ods_raw_jobs (entity_id VARCHAR, is_latest BOOLEAN, job_title VARCHAR, salary_min_k INTEGER, salary_max_k INTEGER, crawled_at TIMESTAMP)")
...
class TestCheckpoint:
    def test_save_and_load(self):
        ...
        # 完成后再加载, 应仍然返回最后进度
        offset2, processed2 = cm.load("run_001", "fetch", "validate")
        assert offset2 == 50
