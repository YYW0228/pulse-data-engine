"""
boss_fetcher_v3.py — BOSS直聘采集 (Mac + Brave)
...
def fetch_jobs(
    ...
    """采集 BOSS直聘（用 Brave 真实浏览器 + 复制用户数据）"""
    ...
    # 检测 Brave 是否在运行 — 自动关掉
    if is_brave_running():
        logger.info("Brave 正在运行, 自动退出...")
        subprocess.run(["osascript", "-e", 'tell application "Brave Browser" to quit'], timeout=10)
        import time
        time.sleep(2)
        # 再确认
        if is_brave_running():
            logger.warning("Brave 未能自动退出, 请手动关闭后重试")
            return []

    # 复制用户数据到临时目录 (避免锁冲突)
    logger.info("复制 Brave 用户数据到临时目录...")
    tmp_dir = Path(tempfile.mkdtemp(prefix="brave_profile_"))
    try:
        # 只复制关键的 Login Data 和 Cookies
        for sub in ["Default/Cookies", "Default/Login Data", "Local State"]:
            src = BRAVE_USER_DATA / sub
            dst = tmp_dir / sub
            if src.exists():
                dst.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(src, dst)
                logger.info(f"  复制 {sub}")
