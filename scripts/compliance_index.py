...
def split_markdown(text: str, min_chars: int = 50) -> list[dict]:
    ...
    def _importance(title: str, content: str) -> float:
        ...
        # 法规/条款实体
        if re.search(
            r"第[一二三四五六七八九十\d]+条|办法|规定|指南|条例|法律|办法|标准",
            title + content[:500],
        ):
            score += 0.15
        # 时效性 (近3年)
...
def index_docs(
    source: Path, rebuild: bool = False, include_jsonl: bool = False, db: str | None = None
) -> dict:
    """索引目录下所有 md 文档 + 生成向量
    ...
    con.execute("""
        CREATE TABLE IF NOT EXISTS compliance_chunks (
            ...
            domain VARCHAR DEFAULT 'compliance'
        )
    """)
    if rebuild:
        con.execute("DELETE FROM compliance_chunks")
    # 兼容旧表: 缺 fetched_at/domain 列 → ALTER 补齐 (不丢数据)
    cols = [
        r[0]
        for r in con.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_name='compliance_chunks'"
        ).fetchall()
    ]
    ...
    if "domain" not in cols:
        try:
            con.execute(
                "ALTER TABLE compliance_chunks ADD COLUMN domain VARCHAR DEFAULT 'compliance'"
            )
            print("→ 兼容: compliance_chunks 补 domain 列")
        except Exception:
            pass
    ...
    for f in files:
        ...
        # domain: 优先读同名 metadata json (ingest 产物), 默认 compliance
        domain = "compliance"
        meta_file = f.with_suffix(".json")
        if meta_file.exists():
            try:
                meta = json.loads(meta_file.read_text(encoding="utf-8"))
                domain = meta.get("domain", "compliance")
        ...
        fetched_at = _dt.datetime.fromtimestamp(f.stat().st_mtime, tz=_dt.timezone.utc).isoformat()
...
def main():
    ...
    parser.add_argument(
        "--db", default=None, help="客户库标识 (独立库: data/customers/<db>/), 默认全局库"
    )
    ...
    result = index_docs(source, rebuild=args.rebuild, include_jsonl=args.include_jsonl, db=args.db)
